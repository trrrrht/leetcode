...
**Other Solution:**
```
void dfs(TreeNode* root, int hDist, int level, map<int, vector<pair<int, int>>>& mp)
    {
        if(root == NULL) return;
        mp[hDist].push_back({level, root->val});
        dfs(root->left, hDist - 1, level + 1, mp);
        dfs(root->right, hDist + 1, level + 1, mp);
    }
